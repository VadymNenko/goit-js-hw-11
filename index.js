...
`).join("");l.insertAdjacentHTML("beforeend",o),g.refresh()}function y(){l.innerHTML=""}function b(){u.classList.remove("hidden")}function L(){u.classList.add("hidden")}const w=document.querySelector(".form");w.addEventListener("submit",S);function S(s){s.preventDefault();const o=s.currentTarget,a=new FormData(o).get("search-text").trim();a&&(o.reset(),b(),y(),p(a).then(e=>{if(e.hits.length===0){n.error({title:"Error",message:"Sorry, there are no images matching your search query. Please, try again!",position:"topRight",pauseOnHover:!0});return}h(e.hits)}).catch(e=>{console.error("Error fetching images:",e),n.error({title:"Error",message:"Failed to fetch images. Please try again later!",position:"topRight",timeout:5e3})}).finally(()=>{L()}))}
